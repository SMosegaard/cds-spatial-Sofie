...
pacman::p_load(knitr, rmdformats, tidyverse, sf, raster)
...
plot(viking, add = TRUE, pch = 1, col = "red")
```
...
# Join the viking monuments with the municipalities to add municipality information
...
viking_neigh %>% 
  group_by(NAME_2) %>% 
  tally() %>% 
  arrange(desc(n)) # Show to number (n) of monuments pr "kommune" in a descending order
...
plot(municipalities_25832[select(st_intersection(least, municipalities_25832, sparse = FALSE)), ]$geometry, col = "red", add = T) 

```

```{r}

saveRDS(viking_neigh, "/Users/Administrator/Documents/Semester6/Spatial/cds-spatial_Sofie/Week05/viking.rds")
