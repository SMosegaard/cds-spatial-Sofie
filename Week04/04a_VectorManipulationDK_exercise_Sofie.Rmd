...
pacman::p_load(sf, raster, tidyverse)
...
plot(st_geometry(municipalities_25832), col = "grey", border = "white")
plot(shelters, add = TRUE, pch = 1, col = "forestgreen")
...
all.equal(crs(shelters), crs(aarhus_mun)) # = TRUE
...
length(sh_buffers) = # 19

# Plot the shelter buffers
plot(sh_buffers)

# Dissolve the buffer features with st_union
sh_buf_union <- st_union(sh_buffers)

# Compute the number of features in sh_buf_union
length(sh_buf_union) # = 1 (here we cluster the individual points into a MULTIPOINT geometry)
...
# Compute the tight bounding box (its "fitted" instead of just a box around it)
shelters_hull <- st_convex_hull(sh1)

# Plot the points together with the hull
plot(shelters_hull, col = "lightpink")
plot(sh1, col = "darkred", add = TRUE)
```
