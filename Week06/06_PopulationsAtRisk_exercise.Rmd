...
pacman::p_load(knitr, rmdformats, tidyverse, sf, mapboxapi, raster, mapview, leaflet)
...
hh_mun <- hh %>% 
  group_by(V2) %>% 
  summarize(sum20=sum(V3), sum21=sum(V4)) %>% 
  rename(municipality = V2, household20 = sum20, household21 = sum21)

# check total households
sum(hh_mun$household20)
sum(hh_mun$household21)

# Change name
hh_mun$municipality[hh_mun$municipality == "Copenhagen"] <- "København"

# join households to no_car dataset and calculate percentages
hh_nocar <- hh_mun %>% inner_join(no_cars, by = "municipality") 
hh_nocar <- hh_nocar %>% 
  mutate(nc2020pct = nc2020/household20*100, 
         nc2021pct = nc2021/household21*100)

# Change names
hh_nocar$municipality[hh_nocar$municipality == "Høje-Taastrup"] <- "Høje Taastrup"
hh_nocar$municipality[hh_nocar$municipality == "Vesthimmerlands"] <- "Vesthimmerland"

```

* Load municipality data either via `getData()` or from a saved object from Week03
* Check and reconcile the names of municipalities are consistent across the `hh_nocar` and the new `municipalities` objects. (Culprits like Aarhus, Vesthimmerlands, and Copenhagen often cause trouble). You need both columns of names to contain the same variants of names else the conflicting ones will be dropped during the following join.
...
municipalities <- st_transform(st_as_sf(municipalities), crs = 25832)

# Change names of the cities, that does not align
municipalities$NAME_2[municipalities$NAME_2 == "Århus"] <- "Aarhus"

# reconcile municipalities' and hh_nocars' problematic names 
#names(municipalities)[7] <- c("municipality") 

# join municipalities with the carless households dataset
no_cars_pct_mun  <-  municipalities %>%
  dplyr::inner_join(hh_nocar, by = c("NAME_2"="municipality")) 
...
regions <- getData(name = "GADM", country = "DNK", level = 1)
regions <- st_transform(st_as_sf(regions), crs = 25832)

central_jutland <- data %>% 
  filter(VARNAME_1 == "Central Jutland")

# Use this single region to clip the municipalities with carless households `no_cars_pct_mun`
no_cars_pct_midt <- st_intersection(central_jutland, no_cars_pct_mun)
...
```{r}
pacman::p_load(opencage)
Sys.setenv(OPENCAGE_KEY = "INSERT KEY")
oc_config()
```

```{r}

# Create object with all the hospitals in Central Jutland

midt_hospitals <- c("Regionshospitalet Brædstrup in Brædstrup","Regionshospitalet Grenaa in Grenaa","Regionshospitalet Hammel Neurocenter in Hammel","Regionshospitalet Herning in Herning","Regionshospitalet Holstebro in Holstebro","Regionshospitalet Horsens in Horsens","Regionshospitalet Kjellerup in Kjellerup","Regionshospitalet Lemvig in Lemvig","Regionshospitalet Odder in Odder","Regionshospitalet Randers in Randers","Regionshospitalet Ringkøbing in Ringkøbing","Regionshospitalet Samsø on the island of Samsø","Regionshospitalet Silkeborg in Silkeborg","Skanderborg Sundhedscenter in Skanderborg","Regionshospitalet Skive in Skive","Regionshospitalet Tarm in Tarm","Regionshospitalet Viborg in Viborg","Aarhus Universitetshospital Aarhus N")

h <- oc_forward_df(placename = midt_hospitals, countrycode = "DK", limit =1, no_annotations = TRUE)
```

```{r}

# Create the cycling range and save the isochrone
...
# We need to define this, so we can plot our polygons on top of the world map
mapbox_map <- leaflet() %>%
...
target25pct <- st_union(target_areas)
mapview(target25pct)
