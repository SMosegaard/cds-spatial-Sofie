...
date: "19/02/2021 updated `r Sys.Date()`"
---
...
1. How many features (rows) does each layer (columns) contain and what kind of geometry are they?
...
2. What is the CRS value in these objects?
  ...
  ''Projected CRS: ETRS89 / UTM zone 32N'' for all except playgrounds which is NA 
...
# Use filter() to limit to over 5ha parks
over5ha <- parks%>%
  filter(areal_m2>50000)

# Count the number of rows
nrow(over5ha) # 17 parks
...
crs_1 <- 4326 # the EPSG number refers to 3D web mercator
st_crs(playgrounds) <- crs_1

# Determine the CRS for the aarhus and elevation rasters
st_crs(aarhus) # 2D (because it does not say longlat - instead it says +proj=utm)
st_crs(elevation) # 3D (because it says +proj=longlat)
...
#Now we see the playgrounds as little dots

#Green larger dots
plotRGB(aarhus, stretch = "lin") # Use plotRGB when there are multiple layers
