...
pacman::p_load(Rcpp, fasterize, mapboxapi)
...
pacman::p_load(leaflet, mapboxapi)
...
Ceres_park <- mb_geocode("Stadion Allé 70, 8000 Aarhus, Denmark")

mapbox_map %>% 
  setView(lng = Ceres_park[1],
          lat = Ceres_park[2], 
          zoom = 17)
...
ceres_iso_cycle <- mb_isochrone(Ceres_park,
                                 profile = "cycling",
                                 time = c(1, 5, 15))
...
plot <- leaflet() %>%
  addMapboxTiles(style_id = "outdoors-v11",
                 username = "mapbox") %>%
  addPolygons(data = ceres_iso_cycle, 
              color = rev(colors),
              fillColor = rev(colors),
              fillOpacity = 0.5, 
              opacity = 0.5, 
              weight = 0.2) %>% 
  addLegend(labels = c(1, 5, 15),
            colors = colors,
            title = "Cycling-time<br/>around Ceres park")
...
# Route from Ceres Park to the bar called Thorkilds in Aarhus

route <- mb_directions(origin = Ceres_park,
                       destination = "Thorkilds, Aarhus, Denmark",
                       profile = "cycling")
...
3. How many and which Viking monuments are reachable within each of the three bands?

```{r}
data <- readRDS("viking.rds")
...
leaflet() %>%
  ...
  addPolygons(data = ceres_iso_cycle, 
              ...
              opacity = 1, 
              weight = 0.2) %>% 
  addCircleMarkers(data = st_transform(viking_aarhus, 4326)) %>%
  addLegend(labels = c(1, 5, 15),
            colors = colors,
            title = "Cycling-time<br/>around Ceres park")
