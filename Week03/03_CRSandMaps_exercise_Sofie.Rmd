...
```{r libraries}
pacman::p_load(sf, raster, tmap, googlesheets4, tidyverse)
```
...
tm_shape(elevation)  +
  tm_raster(title = "elev (masl)", # 5) Add unit information for legend
            style = "cont",
            palette = "RdYlGn") + # 3) Change color palette
  tm_shape(counties) +
  tm_borders(col = "black", # 7) Change border color and size
             lwd = 0.8) +
  tm_scale_bar(breaks = c(0, 50, 100, 150, 200),
               text.size = 1,
               position = c("RIGHT", "top")) +
  tm_compass(position = c("RIGHT", "top"), # 4) Change location of the north arrow
             type = "rose", 
             size = 2) +
  tm_credits(text = "SNM, 20-02-2024") + # 2) Change name and date
  tm_layout(main.title = "Denmark municipalities", # 1) Change title 
            bg.color = "lightblue", # 8) Change background
            inner.margins = c(0, 0, 0, 0))
...
places_sf <- places %>% 
  filter(!is.na(CoordinatesFILLTHIS)) %>%
...
plot(places_sf, add = TRUE) 

# no because they do not have the same crs
...
#nitrates4326 <- nitrates_sf %>% st_set_crs(4326)
nitrates4326 <- st_transform(nitrates25832, crs = 4326)

# Option 2:

the_crs <- crs(nitrates_4326)

elevation4326 <- projectRaster(elevation, crs = the_crs)

# st_transform = transform vector
# projectRaster = transform raster

# Plot the nitrate samples and DK places over the elevation map of Denmark. 
...
```{r your-map}
 ...
 <- getData(name="", country = "", mask = TRUE)
